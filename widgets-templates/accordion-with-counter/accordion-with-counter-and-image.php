<?php function get_accordion_with_counter_image_template($settings, $attributes)
{
  ...
  <section id="accordion-with-counter-image" class="accordion-with-counter accordion-with-counter-tabs-block">
    <div class="accordion-with-counter-format-container">
      <div class="accordion-with-counter-tabs_box">
        <div class="accordion-with-counter-tabs_wrapper">
          <?php $counter_tab = 1;
          foreach ($settings['accordion'] as $item_tab) { ?>
            ...
            <label for="tab-<?php echo esc_attr($counter_tab); ?>" class="accordion-with-counter-tabs_label">
              <?php if ($settings['accordion_counter'] || $settings['accordion_divider']) { ?>
                <span class="accordion-with-counter-tabs_counter">
                    <?php if ($settings['accordion_counter']) {
                      if ($counter_tab < 10)
                        echo '0';
                      echo wp_kses($counter_tab, []);
                    }

                    if ($settings['accordion_divider']) { ?>
                      <span class="accordion-with-counter-tabs_divider">/</span>
                    <?php } ?>
                  </span>
              <?php }

              echo wp_kses_post($item_tab['accordion_name']); ?>
            </label>
            <?php $counter_tab++;
          } ?>
        </div>
